...
namespace Abilities.Systems.Active.Garlic
{
    public class GarlicAbilitySystem : IEcsRunSystem
    {
        private EcsWorld _world;

        private EcsFilter<InitPlayerTag> _playersFilter;
        private EcsFilter<GarlicAbilityTag, InitAbilityTag>.Exclude<CooldownComponent> _garlicAbilityFilter;
        ...
        public void Run()
        {
            ...
            if(_garlicAbilityFilter.GetEntitiesCount() == 0)
                return;
            
            var playerEntity = _playersFilter.GetEntity(0);

            var abilityEntity = _garlicAbilityFilter.GetEntity(0);
        ...
        private Vector3 GetSpawnPoint(Transform playerTransform, float currentRotationDegree, float area) =>
            playerTransform.position + new Vector3(Mathf.Cos(currentRotationDegree * Mathf.Deg2Rad), Mathf.Sin(currentRotationDegree * Mathf.Deg2Rad), 0).normalized * area;
    }
}